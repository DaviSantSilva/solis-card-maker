...
> Botões vivem numa âncora (escala 1:1) 2.8 unidades abaixo (Z) de cada
...
| 5.2 | Zona de deck tem exatamente 1 botão | "Comprar até 5" — sem botões de compra ajustável (removidos) |
| 5.3 | Zona de descarte tem exatamente 2 botões | "Descartar Mão" (linha de cima) e "Refazer Deck" (linha de baixo) |
...
| 5.7 | "Refazer Deck" funciona | Move todo o descarte para a posição do deck e embaralha |
| 5.8 | "Descartar Mão" funciona | Localiza cartas reais por proximidade da posição de mão (`getObjects()` + distância) — **não** `player.getHandObjects()` (mão oculta do TTS, nunca usada neste sistema; era um bug real corrigido) |
| 5.9 | Botões usam função global única por corp+ação, não um despachante por "id" | `createButton()` chama `click_function` com `(objeto, cor, clique_alt)` — **sem** parâmetro de id. `registerHandlersForCorp()` gera `onDraw5_<corp>` etc. dinamicamente, mesmo padrão de `market-manager.lua` |
| 5.11 | Ações do hand-manager são bloqueadas enquanto o setup ainda está rodando em background | `Global.call("isSetupRunning")` checado no início de `drawToHandZone`, `discardHand`, `rebuildDeck` |
| 5.12 | Ações se recuperam sozinhas de "owned by different scripts" após reload de mesa salva | Operações de risco envolvidas em `pcall()`, com retry automático em `drawToHandZone` |
| 5.13 | Cartas compradas chegam com a face para cima | `rotation = { 0, 180, 0 }` explícito em `takeObject`/`setPositionSmooth` |
| 5.15 | Jogador de uma corp não consegue usar botões de outra corp | `blockIfWrongCorp(corp, playerColor)` checado em todos os handlers |
| 5.18 | "Refazer Deck" move as cartas com a face para baixo | `rotation = {0, 180, 180}` explícito (rotZ=180 = verso visível) |
| 5.19 | "Comprar até 5" compra só a diferença necessária, nunca 5 cartas incondicionalmente | `handCounts[corp]` rastreado internamente (não detecção física em tempo real — ver nota abaixo) — compra `5 - atual` |
| 5.20 | Aviso correto quando a mão já tem 5+ cartas | Mensagem "Você já tem 5 cartas, {nome}" via `player.steam_name`, sem tentar comprar |

**Nota sobre `handCounts`:** duas abordagens de detecção física em tempo real foram tentadas e abandonadas por instabilidade — `Physics.cast` (hits inconsistentes entre chamadas quase idênticas) e `getObjects()`+distância como fonte de verdade a cada clique (também não bateu de forma confiável). A solução adotada rastreia a contagem internamente: incrementa a cada compra bem-sucedida, zera ao descartar a mão. Determinístico enquanto o jogador usar os botões — pode desatualizar se alguém arrastar carta manualmente para fora da mão sem usar "Descartar Mão".
...
- **BUG REAL encontrado por trás disso: a flag de trava resetava ~8s antes da hora.** `isSpawning = false` estava sendo setado IMEDIATAMENTE após chamar `mergeAllPendingDecks()` — mas essa função só AGENDA uma cadeia de `Wait.time` (não bloqueia), então a flag voltava a `false` segundos antes da cadeia de verdade terminar (~8.6s no total: 0.6+1+2+5). A trava `isSetupRunning()` no `hand-manager.lua` nunca protegia a janela real de risco por causa disso — não era um problema do hand-manager, era o próprio Global "mentindo" sobre já ter terminado. Fix: mover `isSpawning = false` para dentro do ÚLTIMO `Wait.time` da cadeia inteira, junto com `onCloseClick()`. Regra geral: quando uma flag de "operação em andamento" é usada para sincronizar com outro script, garantir que ela só reseta no fim de TODA a cadeia assíncrona real, não no primeiro `Wait.time` que a dispara.
- **Tolerância de busca inconsistente entre diagnóstico e gameplay real causa falso "vazio".** O diagnóstico pós-spawn usava uma caixa de busca ampla (`findPileWide`, 3x6x3, alcance 6) e sempre encontrava os decks corretamente. A função usada pela COMPRA de verdade (`findPileAt`) tinha uma caixa muito mais estreita (1x1x1, alcance 1) — se o deck assentasse levemente fora do ponto exato de spawn depois de embaralhar (física normal, nada de errado), a busca estreita perdia ele completamente, retornando "nenhuma carta disponível" mesmo com o deck genuinamente ali (confirmado pelo próprio diagnóstico). Regra geral: qualquer função de detecção usada em gameplay real deve ter a MESMA tolerância (ou maior) que a função usada para diagnóstico/debug — nunca mais estreita.
- **Detecção física em tempo real de cartas espalhadas (leque de mão) é fundamentalmente instável — prefira rastrear internamente.** Tentativas com `Physics.cast` (hits inconsistentes entre chamadas quase idênticas ao mesmo estado real) e `getObjects()`+filtro de distância (também não bateu de forma confiável) foram abandonadas para CONTAR quantas cartas existem na zona de mão. A solução robusta foi parar de tentar detectar fisicamente e RASTREAR a contagem internamente em uma tabela Lua (`handCounts[corp]`), incrementada a cada compra bem-sucedida e zerada ao descartar — persistida via `onSave`/`onLoad`. Determinístico, mas assume que o jogador só altera a mão através dos botões do próprio script (uma carta arrastada manualmente para fora sem usar "Descartar Mão" desatualiza a contagem).
- **`player.getHandObjects()` é a mão OCULTA do TTS — nunca use se o sistema usa uma zona de mão física.** Bug real encontrado: `discardHand()` chamava `player.getHandObjects()` para descartar, mas esse sistema nunca colocou cartas na mão oculta do TTS (sempre usou uma zona física na mesa, com `takeObject`/posição explícita). O botão "Descartar Mão" provavelmente nunca funcionou de fato até esse bug ser corrigido — substituído por `getObjects()` + filtro de distância da posição de mão, a mesma técnica usada em outros lugares deste arquivo.
