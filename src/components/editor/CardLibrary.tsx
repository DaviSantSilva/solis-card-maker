...
function groupLibrary(library: SolisCard[]): LibraryItem[] {
...
function CardItem({ card, isActive, onLoad, onDuplicate, onDelete }: {
  ...
  return (
    ...
    >
      ...
      {/* badge cor do jogador (cartas iniciais com variante) */}
      {card.playerColor && (
        <span
          className="absolute right-1 top-1 h-3 w-3 rounded-full border border-neutral-900"
          style={{ background: card.playerColor }}
          title="Cor do jogador"
        />
      )}
    ...
    <div className="flex flex-col gap-1 rounded-lg border border-neutral-800 bg-neutral-800/20 p-2">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-2">
          ...
          {item.cards.map((c) => c.playerColor && (
            <span
              key={c.id}
        ...
        <div className="flex flex-col gap-2 pt-1">
          {item.cards.map((card) => (
            <CardItem
              ...
              onDelete={() => {
                if (confirm(`Apagar variante "${card.name}" (${card.companyId})?`))
                  onDelete(card.id);
              }}
            />
          ))}
...
export function CardLibrary() {
  ...
  const items = groupLibrary(library);
    ...
    <aside className="flex h-full w-52 shrink-0 flex-col border-l border-neutral-800 bg-neutral-900">
      ...
      <div className="flex items-center justify-between border-b border-neutral-800 px-3 py-3">
        <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-500">
          ...
          {groups > 0 && (
            <span className="ml-1 text-neutral-600">({groups} grupo{groups !== 1 ? "s" : ""})</span>
          )}
        </p>
      </div>
        ...
        {items.map((item) =>
          item.type === "single" ? (
            <CardItem
              ...
              onDelete={() => {
                if (confirm(`Apagar "${item.card.name}"?`)) deleteCard(item.card.id);
              }}
            />
          ) : (
            <VariantGroup
